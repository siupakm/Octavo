...
(
    output  wire    [(((               A_IO_READ_PORT_COUNT)  + (                    SIMD_A_IO_READ_PORT_COUNT  * SIMD_LANE_COUNT)) * MESH_LINE_NODE_COUNT)-1:0] A_rden,
    output  reg     [(((A_WORD_WIDTH * A_IO_READ_PORT_COUNT)  + (SIMD_A_WORD_WIDTH * SIMD_A_IO_READ_PORT_COUNT  * SIMD_LANE_COUNT)) * MESH_LINE_NODE_COUNT)-1:0] A_in,
    output  wire    [(((               A_IO_WRITE_PORT_COUNT) + (                    SIMD_A_IO_WRITE_PORT_COUNT * SIMD_LANE_COUNT)) * MESH_LINE_NODE_COUNT)-1:0] A_wren,
    output  wire    [(((A_WORD_WIDTH * A_IO_WRITE_PORT_COUNT) + (SIMD_A_WORD_WIDTH * SIMD_A_IO_WRITE_PORT_COUNT * SIMD_LANE_COUNT)) * MESH_LINE_NODE_COUNT)-1:0] A_out, 
    
    output  wire    [(((               B_IO_READ_PORT_COUNT)  + (                    SIMD_B_IO_READ_PORT_COUNT  * SIMD_LANE_COUNT)) * MESH_PAGE_LINE_COUNT)-1:0] B_rden,
    output  reg     [(((B_WORD_WIDTH * B_IO_READ_PORT_COUNT)  + (SIMD_B_WORD_WIDTH * SIMD_B_IO_READ_PORT_COUNT  * SIMD_LANE_COUNT)) * MESH_PAGE_LINE_COUNT)-1:0] B_in,
    output  wire    [(((               B_IO_WRITE_PORT_COUNT) + (                    SIMD_B_IO_WRITE_PORT_COUNT * SIMD_LANE_COUNT)) * MESH_PAGE_LINE_COUNT)-1:0] B_wren,
    output  wire    [(((B_WORD_WIDTH * B_IO_WRITE_PORT_COUNT) + (SIMD_B_WORD_WIDTH * SIMD_B_IO_WRITE_PORT_COUNT * SIMD_LANE_COUNT)) * MESH_PAGE_LINE_COUNT)-1:0] B_out
);
    integer     cycle;
    reg         clock;
    reg         half_clock;
    ...
    localparam MESH_NODE_COUNT = MESH_LINE_NODE_COUNT * MESH_PAGE_LINE_COUNT;
...
TOP_LEVEL_MODULE="${MESH_NAME}_test_bench"
...
OCTAVO="$$INSTALL_BASE/Octavo/Misc/params.v \\
        ...
        $$INSTALL_BASE/Mesh/Mesh_Pipe_Array.v \\
        $$INSTALL_BASE/Mesh/Mesh_Node_Line.v \\
        $$INSTALL_BASE/Mesh/Mesh_Page.v \\
        ../${MESH_NAME}.v
"
